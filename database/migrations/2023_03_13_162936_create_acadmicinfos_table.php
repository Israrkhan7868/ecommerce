<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up()
{
    Schema::create('acadmicinfos', function (Blueprint $table) {
        $table->id();
        $table->unsignedBigInteger('basicinfo_id');
        $table->foreign('basicinfo_id')->references('id')->on('basicinfos')->onDelete('cascade');
        $table->string('coursetitle')->nullable();
        $table->string('coursefee')->nullable();
        $table->string('feestatus')->nullable();
        $table->string('startingdate')->nullable();
        $table->string('batchno')->nullable();
        $table->string('coursestatus')->nullable();
        $table->timestamps();
    });
}


    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('acadmicinfos');
    }
};
